...
dimensions = (1024, 512)
# List all the algo available in the project in dictionary and call the
#  necessary functions from algo.py
algo = {"SelectionSort": algo.SelectionSort(), "BubbleSort": algo.BubbleSort(), "InsertionSort": algo.InsertionSort(), "MergeSort": algo.MergeSort(), "QuickSort": algo.QuickSort()}
...
def main(args):
    ...
    # Case: user requests list of algo
    elif args[1] == "list":
            print("Available algo:\n\t" + "\n\t".join(algo.keys()))
            sys.exit(0)
    # Case: user selected an algorithm
    else:
        try:
            algorithm = algo[args[1]] # Collect algorithm
